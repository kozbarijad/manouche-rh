...
bg.innerHTML = `<div class="modal"> <div class="modal-h"> <h3>${esc(title)}</h3> <button class="btn-ghost" data-close aria-label="Fermer">✕</button> </div> <div class="modal-b">${body}</div> ${footer ?`<div class="modal-f">${footer}</div>` : ''} </div>`;
...
return `<div class="shell"> <header class="topnav"> <div class="topnav-inner"> <div class="topnav-brand"> <span class="brand-mark">M</span> <span class="brand-name">Man'ouché</span> </div> <nav class="topnav-links"> ${TOP_SECTIONS.map(s => { const active = s.key === currentSection.key; const firstPage = s.pages[0]; return`<button class="topnav-link ${active?'active':''}" data-page="${firstPage}">${esc(s.label)}${s.badge > 0 ? `<span class="dot-badge">${s.badge}</span>` : ‘’}</button>`; }).join('')} </nav> <div class="topnav-right"> <button class="topnav-icon" title="${state.fbReady?'Connecté':'Hors ligne'}"> <span class="status-dot-tiny" style="background:${state.fbReady?'#22c55e':'#737373'};"></span> </button> ${anomalies.length ? `<button class="topnav-icon" data-page="alerts" title="${anomalies.length} alertes">
...
</button>` : ''} <button class="topnav-icon" data-logout title="Déconnexion"> <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4M16 17l5-5-5-5M21 12H9"/></svg> </button> <button class="topnav-avatar" id="openAdminSettings" title="Paramètres admin">A</button> </div> </div> <div class="topnav-mobile"> <select id="mobNav"> <optgroup label="Tableau de bord"> <option value="dashboard" ${state.page==='dashboard'?'selected':''}>Aperçu</option> </optgroup> <optgroup label="Planning"> <option value="planning" ${state.page==='planning'?'selected':''}>Planning hebdo</option> <option value="month" ${state.page==='month'?'selected':''}>Vue mois</option> </optgroup> <optgroup label="Équipe"> <option value="employees" ${state.page==='employees'?'selected':''}>Salariés</option> </optgroup> <optgroup label="Heures"> <option value="hours" ${state.page==='hours'?'selected':''}>Préparation paie</option> <option value="pointages" ${state.page==='pointages'?'selected':''}>Pointages</option> </optgroup> <optgroup label="Conformité"> <option value="rappels" ${state.page==='rappels'?'selected':''}>Rappels${rappelsCount?` (${rappelsCount})`:''}</option> <option value="emargements" ${state.page==='emargements'?'selected':''}>Émargements${unsignedCount?` (${unsignedCount})`:''}</option> <option value="etablissement" ${state.page==='etablissement'?'selected':''}>Établissement</option> </optgroup> <optgroup label="RH"> <option value="requests" ${state.page==='requests'?'selected':''}>Demandes d'absence${pendingReqs?` (${pendingReqs})`:''}</option> <option value="cp" ${state.page==='cp'?'selected':''}>Compteurs CP</option> <option value="rh" ${state.page==='rh'?'selected':''}>Suivi salariés</option> <option value="alerts" ${state.page==='alerts'?'selected':''}>Alertes${anomalies.length?` (${anomalies.length})`:’’}</option>
</optgroup>
</select>
</div>
</header>
...
return `<table class="tbl"> <thead><tr><th>Salarié</th><th>Horaire</th><th>Type</th><th>Heures</th></tr></thead> <tbody> ${onShift.map(({e, shifts}) => shifts.map(s => `
<tr>
...
return ` <div class="alert-row ${a.severity === 'warn' ? 'warn' : ''}"> <div class="alert-icon">!</div> <div class="alert-body"> <div class="alert-title">${esc(a.title)}</div> <div class="alert-meta">${esc(a.detail)}</div> </div> </div>`;
}
...
return `<div class="page-head"> <div> <div class="uppercase-eyebrow">Planning</div> <h1 class="h-1">${fmtRange(state.weekStart, wkEnd)}</h1> <div class="row" style="gap:8px;margin-top:10px;flex-wrap:wrap;"> <span class="chip">${Math.round(totalH)} h planifiées</span> ${isPublished ?`<span class="chip good">✓ Publié ${new Date(pub.publishedAt).toLocaleDateString(‘fr-FR’,{day:‘numeric’,month:‘short’,hour:‘2-digit’,minute:‘2-digit’})}</span>`: (weekShiftCount > 0 ?`<span class="chip draft">● Brouillon — invisible des salariés</span>`:`<span class="chip">Semaine vide</span>`)} ${schoolHolidaysOverlappingWeek(state.weekStart).map(h => `<span class="chip accent" title="Zone C · ${new Date(h.start).toLocaleDateString('fr-FR')} → ${new Date(h.end).toLocaleDateString('fr-FR')}">🎒 Vacances ${esc(h.name)}</span>`).join('')} ${weekFeries.map(f =>`<span class="chip warn">📅 ${esc(f.short)} (${DAYS_SHORT[f.dayIdx]})</span>`
).join(’’)}
</div>
</div>
...
return `<div class="shift ${s.type}"> <div class="shift-title">${esc(label)}</div> <div class="shift-time">${s.start} – ${s.end}${pauseStr}</div> <div class="shift-dur">${durStr}</div> </div>`;
}
...
const footer = `${existing.length ?`<button class="btn-danger" id="shDel">Supprimer</button>`: ''} <div class="spacer"></div> <button class="btn-sec" data-close>Annuler</button> <button class="btn-pri" id="shSave" style="width:auto;">Enregistrer</button>`;
...
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Prénom *</label><input class="input" id="ncPrenom"></div> <div class="field"><label class="field-label">Nom *</label><input class="input" id="ncNom"></div> <div class="field"><label class="field-label">Poste</label><input class="input" id="ncPoste" value="Cuisinier"></div> <div class="field"><label class="field-label">Contrat</label> <select class="input" id="ncContrat"> ${['CDI','CDD','Extra','Apprenti','Stage'].map(c =>`<option>${c}</option>`).join('')} </select> </div> <div class="field"><label class="field-label">Heures hebdo</label><input class="input mono" id="ncHeures" type="number" value="35"></div> <div class="field"><label class="field-label">Email</label><input class="input" id="ncEmail" type="email"></div> </div> <div class="text-mute" style="margin-top:12px;font-size:12px;">Les autres infos (état civil, adresse, etc.) se complètent depuis la fiche du salarié.</div> `;
const footer = `<div class="spacer"></div> <button class="btn-sec" data-close>Annuler</button> <button class="btn-pri" id="ncSave" style="width:auto;padding:10px 18px;">Créer</button>`;
...
return `<div class="info-grid"> <div class="panel"> <div class="panel-head"><h3>État civil</h3></div> <div class="panel-body"> ${kvRow('Genre', n.genre || 'Non renseigné')} ${kvRow('Prénom', n.prenom)} ${kvRow('Nom de naissance', n.nomNaissance || 'Non renseigné')} ${kvRow('Nom de famille', n.nom)} ${kvRow('Nationalité', n.nationalite || 'Non renseigné')} ${kvRow('Date de naissance', n.dateNaissance ? fmtDateShort(n.dateNaissance) : 'Non renseigné')} ${kvRow('Pays de naissance', n.paysNaissance || 'Non renseigné')} ${kvRow('Département de naissance', n.departementNaissance || 'Non renseigné')} ${kvRow('Commune de naissance', n.communeNaissance || 'Non renseigné')} </div> </div> <div class="panel"> <div class="panel-head"><h3>Coordonnées</h3></div> <div class="panel-body"> ${kvRow('Email', n.email || 'Non renseigné')} ${kvRow('Tél. mobile', n.telMobile || 'Non renseigné')} ${kvRow('Notifications SMS', n.notifSMS ? 'Oui' : 'Non')} ${kvRow('Tél. fixe', n.telFixe || 'Non renseigné')} ${kvRow('Adresse', n.adresse || 'Non renseigné')} ${kvRow('Complément', n.complementAdresse || 'Non renseigné')} ${kvRow('Code postal', n.codePostal || 'Non renseigné')} ${kvRow('Ville', n.ville || 'Non renseigné')} ${kvRow('Pays', n.pays || 'France')} </div> </div> </div> <div class="row" style="justify-content:center;margin-top:18px;"> <button class="btn-pri" id="editInfo" style="width:auto;padding:10px 18px;">✎ Modifier les informations personnelles</button> </div>`;
}
...
return `<div class="panel"> <div class="panel-head"><h3>Contrat en cours</h3></div> <div class="panel-body"> ${kvRow('Type', n.contrat || 'Non renseigné')} ${kvRow('Début du contrat', n.contratDebut ? fmtDateShort(n.contratDebut) : 'Non renseigné')} ${kvRow('Fin du contrat', n.contratFin ? fmtDateShort(n.contratFin) : (n.contrat==='CDI'?'Indéterminée':'Non renseigné'))} ${kvRow('Période d\'essai', n.periodeEssaiJours ?`${n.periodeEssaiJours} jours${essaiFin ? ` · échéance ${fmtDateShort(dateISO(essaiFin))}` : ‘’}`: 'Non renseigné')} ${kvRow('Poste', n.poste || 'Non renseigné')} ${kvRow('Rémunération mensuelle brute', n.remuneration ?`${n.remuneration} €`: 'Non renseigné')} ${kvRow('Taux horaire', n.taux ?`${n.taux} €/h`: 'Non renseigné')} ${kvRow('Durée de travail hebdomadaire',`${n.heures || 0}h`)} ${kvRow('Nb. de jours travaillés par semaine', `${n.joursTravailles || 5} jours`)} ${kvRow('Remboursement Navigo / mois', n.navigoMensuel ? `${n.navigoMensuel} €` : ‘Non renseigné’)}
</div>
</div>
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Type de contrat</label> <select class="input" id="cType"> ${['CDI','CDD','Extra','Apprenti','Stage','Interim'].map(c =>`<option ${c===n.contrat?‘selected’:’’}>${c}</option>`).join('')} </select> </div> <div class="field"><label class="field-label">Poste</label><input class="input" id="cPoste" value="${esc(n.poste)}"></div> <div class="field"><label class="field-label">Date de début</label><input class="input" id="cDebut" type="date" value="${esc(n.contratDebut)}"></div> <div class="field"><label class="field-label">Date de fin (laisser vide pour CDI)</label><input class="input" id="cFin" type="date" value="${esc(n.contratFin)}"></div> <div class="field"><label class="field-label">Période d'essai (jours)</label><input class="input mono" id="cEssai" type="number" value="${n.periodeEssaiJours||60}"></div> <div class="field"><label class="field-label">Heures hebdo</label><input class="input mono" id="cHeures" type="number" value="${n.heures||35}"></div> <div class="field"><label class="field-label">Jours travaillés / semaine</label><input class="input mono" id="cJours" type="number" min="1" max="7" value="${n.joursTravailles||5}"></div> <div class="field"><label class="field-label">Taux horaire (€)</label><input class="input mono" id="cTaux" type="number" step="0.01" value="${n.taux||12}"></div> <div class="field full"><label class="field-label">Rémunération mensuelle brute (€)</label><input class="input mono" id="cRem" type="number" step="0.01" value="${n.remuneration||0}"></div> <div class="field full"><label class="field-label">Remboursement Navigo / transport mensuel (€)</label><input class="input mono" id="cNavigo" type="number" step="0.01" value="${n.navigoMensuel||0}" placeholder="ex: 42,15"></div> </div> `;
const footer = `<div class="spacer"></div> <button class="btn-sec" data-close>Annuler</button> <button class="btn-pri" id="cSave" style="width:auto;padding:10px 18px;">Enregistrer</button>`;
...
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Date</label><input class="input" id="aDate" type="date" value="${dateISO(new Date())}"></div> <div class="field"><label class="field-label">Type</label> <select class="input" id="aType"> <option>Changement d'horaire</option> <option>Augmentation</option> <option>Changement de poste</option> <option>Renouvellement CDD</option> <option>Autre</option> </select> </div> <div class="field full"><label class="field-label">Modification (résumé court)</label><input class="input" id="aModif" placeholder="ex: passage 35h → 39h"></div> <div class="field full"><label class="field-label">Détail (optionnel)</label><textarea class="input" id="aDetail" rows="3"></textarea></div> </div>`;
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const flag = (id, checked, label, detail) => `<label class="modalite-row"> <input type="checkbox" id="${id}" ${checked?'checked':''}> <div> <div class="modalite-label">${label}</div> <div class="modalite-detail">${detail}</div> </div> </label>`;
const body = `<div class="text-mute" style="font-size:12.5px;line-height:1.5;margin-bottom:18px;"> Ces réglages adaptent le calcul de la paie pour ce salarié. Ils s'appliquent automatiquement dans l'onglet Heures et les compteurs. </div> ${flag('mpSansHS', n.sansHeuresSupp, 'Sans heures supplémentaires', 'Toutes les heures comptées au taux normal — pas de majoration +25% / +50%. Utile pour un forfait ou un accord spécifique.')} ${flag('mpForfait', n.forfaitJours, 'Forfait jours', 'Cadre au forfait — pas de décompte horaire détaillé, rémunération forfaitaire.')} ${flag('mpExclureCP', n.exclureCP, 'Sans gestion de congés payés', 'Ce salarié n\'apparaît pas dans les compteurs CP. Pour les extras payés à la mission ou statuts particuliers.')} ${flag('mpPayeNet', n.payeNet, 'Rémunération exprimée en net', 'Le taux horaire saisi est un montant net (et non brut). Affiché avec la mention "net".')} ${flag('mpSansRepas', n.sansAvantageRepas, 'Sans avantage en nature repas', 'Ne pas calculer l\'avantage repas HCR (4,15€/repas) pour ce salarié.')} ${flag('mpSansNavigo', n.sansNavigo, 'Sans remboursement transport', 'Pas de prise en charge Navigo, même si un montant est renseigné dans le contrat.')} ${flag('mpExclurePaie', n.exclurePaie, 'Exclure du récap paie', 'Ce salarié n\'apparaît pas du tout dans l\'onglet Heures. Pour un gérant non salarié, un stagiaire non rémunéré, etc.')}`;
const footer = `<button class="btn-sec" data-close>Annuler</button> <button class="btn-pri" id="mpSave" style="width:auto;">Enregistrer</button>`;
...
return `<div class="info-grid"> <div class="panel"> <div class="panel-head"><h3>Temps de travail</h3></div> <div class="panel-body"> ${kvRow('Mois en cours',`${monthH.toFixed(1)} h`)} ${kvRow('Heures contractuelles hebdo', `${n.heures||0} h`)} ${kvRow('Estimation mensuelle contractuelle', `~${((n.heures||0) * 4.33).toFixed(0)} h`)} </div> </div> <div class="panel"> <div class="panel-head"><h3>Disponibilités hebdomadaires</h3></div> <div class="panel-body" style="padding:0;"> ${DAYS.map((d,i) => `
...
`).join('')} </div> </div> </div> <div class="row" style="justify-content:center;margin-top:18px;"> <button class="btn-pri" id="editDispos" style="width:auto;padding:10px 18px;">✎ Modifier les disponibilités</button> </div> `;
}
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const body = `<div class="text-mute" style="margin-bottom:10px;font-size:12.5px;">Décoche les jours où le salarié n'est pas disponible</div> ${DAYS.map((d,i) =>`
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const body = `<div class="modal-section-title">N-1 (année précédente)</div> <div class="form-grid"> <div class="field"><label class="field-label">CP acquis N-1</label><input class="input mono" id="cAcq1" type="number" step="0.5" value="${n.cpAcquisNm1||0}"></div> <div class="field"><label class="field-label">CP pris N-1</label><input class="input mono" id="cPri1" type="number" step="0.5" value="${n.cpPrisNm1||0}"></div> </div> <div class="modal-section-title">N (année en cours)</div> <div class="form-grid"> <div class="field"><label class="field-label">CP acquis N</label><input class="input mono" id="cAcq" type="number" step="0.5" value="${n.cpAcquisN||0}"></div> <div class="field"><label class="field-label">CP pris N (manuel)</label><input class="input mono" id="cPri" type="number" step="0.5" value="${n.cpPrisN||0}"></div> </div> <div class="modal-section-title">Autres compteurs</div> <div class="form-grid"> <div class="field"><label class="field-label">Repos compensateur (heures)</label><input class="input mono" id="cRC" type="number" step="0.5" value="${n.reposCompensateur||0}"></div> <div class="field"><label class="field-label">Récup jours fériés (heures)</label><input class="input mono" id="cRJF" type="number" step="0.5" value="${n.recupJoursFeries||0}"></div> </div>`;
...
return `<div class="row" style="gap:10px;margin-bottom:14px;flex-wrap:wrap;align-items:center;"> <div class="row" style="gap:8px;flex:1;"> <span class="chip">${docs.length} document${docs.length>1?'s':''}</span> <span class="chip">${formatBytes(totalSize)}</span> </div> ${docs.length > 0 ?`<button class="btn-sec" id="downloadAllDocs">↓ Tout télécharger</button>` : ‘’}
...
const e = state.employees.find(x => x.id === empId);
const body = `<div class="form-grid"> <div class="field full"><label class="field-label">Catégorie</label> <select class="input" id="dCat"> ${Object.entries(DOC_CATEGORIES).map(([k,v]) =>`<option value="${k}">${v.icon} ${esc(v.label)}</option>`).join('')} </select> </div> <div class="field full"> <label class="field-label">Fichier(s)</label> <label class="file-drop" id="fileDrop"> <input type="file" id="dFile" multiple accept="image/*,application/pdf,.doc,.docx,.xls,.xlsx" style="display:none;"> <div class="file-drop-content"> <div style="font-size:32px;">📤</div> <div style="font-weight:500;margin-top:8px;">Cliquer pour choisir ou déposer</div> <div class="text-mute" style="font-size:12px;margin-top:4px;">PDF, images, Word, Excel — <strong>max 2 Mo par fichier</strong></div> <div class="text-mute" style="font-size:11px;margin-top:2px;">Les images sont automatiquement compressées</div> </div> </label> <div id="fileList" style="margin-top:10px;"></div> </div> </div> <div id="uploadProgress" style="display:none;margin-top:12px;"> <div class="progress-bar"><div class="progress-fill" id="progFill"></div></div> <div class="text-mute" style="font-size:11.5px;margin-top:6px;" id="progLabel">Upload en cours...</div> </div> `;
...
fl.innerHTML = selectedFiles.map((f,i) => `<div class="row" style="gap:8px;padding:6px 10px;background:#fafafa;border-radius:6px;margin-bottom:4px;font-size:12.5px;"> <span style="flex:1;">${esc(f.name)}</span> <span class="text-mute mono">${formatBytes(f.size)}</span> <button class="btn-ghost" data-rm="${i}" style="padding:0 6px;">✕</button> </div>`).join(’’);
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Username</label><input class="input mono" id="rUser" value="${esc(n.username)}"></div> <div class="field"><label class="field-label">Code PIN</label><input class="input mono" id="rCode" value="${esc(String(n.code))}"></div> <div class="field full"><label class="field-label"><input type="checkbox" id="rConnect" ${n.peutSeConnecter!==false?'checked':''}> Le salarié peut accéder à son compte</label></div> <div class="field full"><label class="field-label"><input type="checkbox" id="rForeign" ${n.travailleurEtranger?'checked':''}> Travailleur étranger</label></div> </div> <div id="foreignWrap" style="${n.travailleurEtranger?'':'display:none;'}"> <div class="modal-section-title">Titre de séjour</div> <div class="form-grid"> <div class="field"><label class="field-label">Type</label><input class="input" id="rTSType" value="${esc(n.titreSejourType)}" placeholder="ex: Carte de séjour"></div> <div class="field"><label class="field-label">Numéro</label><input class="input mono" id="rTSNum" value="${esc(n.titreSejourNumero)}"></div> <div class="field"><label class="field-label">Début validité</label><input class="input" id="rTSDeb" type="date" value="${esc(n.titreSejourDebut)}"></div> <div class="field"><label class="field-label">Fin validité</label><input class="input" id="rTSFin" type="date" value="${esc(n.titreSejourFin)}"></div> </div> </div>`;
...
const e = state.employees.find(x => x.id === state.empDetail);
const n = normEmp(e);
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Date de sortie</label><input class="input" id="eDate" type="date" value="${esc(n.dateSortie)||dateISO(new Date())}"></div> <div class="field"><label class="field-label">Motif</label> <select class="input" id="eMotif"> <option value="">—</option> <option ${n.motifSortie==='Démission'?'selected':''}>Démission</option> <option ${n.motifSortie==='Fin de CDD'?'selected':''}>Fin de CDD</option> <option ${n.motifSortie==='Licenciement'?'selected':''}>Licenciement</option> <option ${n.motifSortie==='Rupture conventionnelle'?'selected':''}>Rupture conventionnelle</option> <option ${n.motifSortie==="Fin de période d'essai"?'selected':''}>Fin de période d'essai</option> <option ${n.motifSortie==='Autre'?'selected':''}>Autre</option> </select> </div> <div class="field full" style="margin-top:8px;"> <label class="field-label"><input type="checkbox" id="eInactif" checked> Marquer le salarié comme inactif</label> </div> </div>`;
...
const e = state.employees.find(x => x.id === empId);
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Type</label> <select class="input" id="abType"> ${Object.entries(LEAVE_TYPES).map(([k,v]) =>`<option value="${k}">${esc(v.label)}</option>`).join('')} </select> </div> <div class="field"><label class="field-label">Date de début</label><input class="input" id="abStart" type="date" value="${dateISO(new Date())}"></div> <div class="field"><label class="field-label">Date de fin</label><input class="input" id="abEnd" type="date" value="${dateISO(new Date())}"></div> <div class="field full"><label class="field-label">Motif (optionnel)</label><textarea class="input" id="abMotif" rows="2"></textarea></div> <div class="field full text-mute" style="font-size:12px;">Cette absence sera automatiquement validée et injectée dans le planning.</div> </div> `;
...
if (rappels.length === 0) {
return `<div class="page-head"> <div> <div class="uppercase-eyebrow">Conformité</div> <h1 class="h-1">Rappels et échéances</h1> </div> </div> <div class="panel"> <div class="panel-body" style="text-align:center;padding:64px 24px;"> <div style="font-size:56px;line-height:1;margin-bottom:16px;">✨</div> <h2 class="h-2" style="margin-bottom:8px;">Tout est à jour</h2> <div class="text-mute">Aucune échéance à anticiper sur les 90 prochains jours.</div> </div> </div>`;
}
...
const todayISO = dateISO(new Date());
const body = `<div style="font-size:13px;color:var(--c-ink-3);line-height:1.55;margin-bottom:18px;"> Marque ici la mise à jour annuelle du DUERP. Le document physique (ou PDF) doit être conservé séparément et présenté à tout salarié, à l'Inspection du travail, à la médecine du travail. </div> <div class="form-grid"> <div class="field full"><label class="field-label">Date de mise à jour</label><input class="input" id="duerpDate" type="date" value="${current.updatedAt ? current.updatedAt.slice(0,10) : todayISO}"></div> <div class="field full"><label class="field-label">Note interne (optionnel)</label><textarea class="input" id="duerpNote" rows="3" placeholder="ex : ajout du risque allergènes, mise à jour TMS cuisine...">${esc(current.note||'')}</textarea></div> </div>`;
...
const body = `<div class="text-mute" style="font-size:13px;line-height:1.55;margin-bottom:18px;"> ${usingDefault ?`<div class="info-banner" style="background:var(--c-warn-bg);border-left:3px solid var(--c-warn-text);padding:12px 14px;border-radius:8px;color:var(--c-warn-dark);font-size:13px;">⚠️ Tu utilises encore les <strong>identifiants par défaut</strong> (<code>${ADMIN_USER}</code> / <code>${ADMIN_CODE}</code>). Change-les immédiatement pour sécuriser ton accès.</div>`:`Identifiant actuel : <strong>${esc(current.username)}</strong> · Dernière modification : ${current.updatedAt ? new Date(current.updatedAt).toLocaleDateString(‘fr-FR’) : ‘—’}`}
</div>
...
const css = `body { font-family: Georgia, serif; max-width: 720px; margin: 30px auto; padding: 0 30px; color: #1a1817; line-height: 1.6; } h1 { font-size: 22px; margin: 30px 0 16px; border-bottom: 2px solid #1a1817; padding-bottom: 8px; } h2 { font-size: 16px; margin-top: 20px; } .doc-section { page-break-after: always; padding-bottom: 40px; } .doc-section:last-child { page-break-after: auto; } .header { display: flex; justify-content: space-between; margin-bottom: 24px; } .from { font-size: 12.5px; } .to { text-align: right; font-size: 12.5px; } .signature-line { margin-top: 50px; display: flex; justify-content: space-between; font-size: 12px; } .signature-line .box { width: 240px; } .signature-line .box strong { display: block; margin-bottom: 40px; font-size: 12.5px; } .signature-line .line { border-top: 1px solid #555; margin-top: 30px; padding-top: 6px; } table { width: 100%; border-collapse: collapse; margin: 14px 0; font-size: 13px; } table th, table td { padding: 8px 10px; border: 1px solid #d0c8b8; text-align: left; } table th { background: #f5f2ea; } table .num { text-align: right; font-family: monospace; } @media print { body { margin: 0; padding: 18mm; } @page { margin: 0; size: A4; } }`;

let html = `<!doctype html><html><head><meta charset="utf-8"><title>Pack sortie ${empNom}</title><style>${css}</style></head><body>`;

if (docs.certif) {
html += `<div class="doc-section"> <div class="header"> <div class="from"> <strong>SAS Man'ouché</strong><br> ${esc(emp.adresseRestau || '')}<br> SIRET / RCS </div> <div class="to"> <strong>${esc(empNom)}</strong><br> ${esc(adresseSal)} </div> </div> <h1>Certificat de travail</h1> <p>Je soussigné, gérant de la société Man'ouché, certifie que <strong>${esc(empNom)}</strong> (né(e) le ${n.dateNaissance ? new Date(n.dateNaissance).toLocaleDateString('fr-FR') : '—'}) a été employé(e) au sein de notre établissement :</p> <ul> <li>en qualité de <strong>${esc(emp.poste || '—')}</strong></li> <li>du <strong>${dateDebutFmt}</strong> au <strong>${dateSortieFmt}</strong> inclus</li> <li>sous contrat de type <strong>${esc(emp.contrat || '—')}</strong></li> </ul> <p>${esc(empNom)} nous quitte ce jour libre de tout engagement.</p> <p>Le présent certificat est délivré pour servir et valoir ce que de droit.</p> <p style="margin-top:24px;">Fait à Paris, le ${todayFmt}.</p> <div class="signature-line"> <div class="box"><strong>Signature de l'employeur</strong><div class="line">Pour la direction</div></div> </div> </div>`;
}

if (docs.stc) {
const totalDu = soldeCp * (n.taux || 12) * 7 + precarite + indemLic;
html += `<div class="doc-section"> <div class="header"> <div class="from"> <strong>SAS Man'ouché</strong><br> SIRET / RCS </div> <div class="to"> <strong>${esc(empNom)}</strong><br> ${esc(adresseSal)} </div> </div> <h1>Reçu pour solde de tout compte</h1> <p>Je soussigné(e), <strong>${esc(empNom)}</strong>, reconnais avoir reçu de la société Man'ouché, employeur, à l'occasion de la rupture de mon contrat de travail (motif : ${esc(motif)}) intervenue le <strong>${dateSortieFmt}</strong>, les sommes suivantes pour solde de tout compte :</p> <table> <tr><th>Élément de rémunération</th><th class="num">Montant brut (€)</th></tr> <tr><td>Solde de congés payés (${soldeCp.toFixed(1)} jours)</td><td class="num">${(soldeCp * (n.taux||12) * 7).toFixed(2)}</td></tr> ${precarite > 0 ?`<tr><td>Indemnité de fin de contrat (précarité)</td><td class="num">${precarite.toFixed(2)}</td></tr>`: ''} ${indemLic > 0 ?`<tr><td>Indemnité légale de licenciement</td><td class="num">${indemLic.toFixed(2)}</td></tr>`: ''} <tr><th>TOTAL BRUT</th><th class="num">${totalDu.toFixed(2)}</th></tr> </table> <p style="font-size:12px;font-style:italic;color:#555;">Le présent reçu peut être dénoncé par le salarié dans un délai de <strong>6 mois</strong> à compter de sa signature (article L1234-20 du Code du travail).</p> <p style="margin-top:24px;">Établi en deux exemplaires originaux, à Paris le ${todayFmt}.</p> <div class="signature-line"> <div class="box"><strong>Signature du salarié</strong> <span style="font-size:11px;">(précédée de "Lu et approuvé, bon pour solde de tout compte")</span><div class="line"></div></div> <div class="box"><strong>Signature de l'employeur</strong><div class="line"></div></div> </div> </div>`;
}

if (docs.attest) {
html += `<div class="doc-section"> <h1>Attestation France Travail</h1> <p style="font-size:13px;color:#666;font-style:italic;">Document brouillon — la version officielle doit être déposée sur <strong>net-entreprises.fr</strong> via la DSN événementielle dans les 5 jours suivant la rupture.</p> <h2>Identification du salarié</h2> <p> <strong>${esc(empNom)}</strong><br> Né(e) le ${n.dateNaissance ? new Date(n.dateNaissance).toLocaleDateString('fr-FR') : '—'}<br> ${esc(adresseSal)}<br> Nationalité : ${esc(n.nationalite || '—')} </p> <h2>Identification de l'employeur</h2> <p> <strong>SAS Man'ouché</strong><br> SIRET : à renseigner<br> Code NAF : 5610A (Restauration traditionnelle)<br> Convention collective : HCR (IDCC 1979) </p> <h2>Période d'emploi</h2> <ul> <li>Du <strong>${dateDebutFmt}</strong> au <strong>${dateSortieFmt}</strong></li> <li>Emploi occupé : <strong>${esc(emp.poste || '—')}</strong></li> <li>Type de contrat : <strong>${esc(emp.contrat || '—')}</strong></li> <li>Durée hebdomadaire : <strong>${emp.heures || 35} h</strong></li> </ul> <h2>Motif de la rupture</h2> <p><strong>${esc(motif)}</strong></p> <h2>Salaires des 12 derniers mois (à compléter par le comptable)</h2> <p style="font-size:12px;color:#666;">Cette section sera renseignée depuis la DSN par le cabinet comptable.</p> <p style="margin-top:24px;">Fait à Paris, le ${todayFmt}.</p> <div class="signature-line"> <div class="box"><strong>Signature de l'employeur</strong><div class="line"></div></div> </div> </div>`;
}
...
return `<div class="panel"> <div class="panel-body ${list.length?'nopad':''}" style="${list.length?'overflow-x:auto':''}"> ${list.length === 0 ?`<div class="text-mute" style="text-align:center;padding:32px 0;font-size:13px;">${esc(emptyMsg)}</div>`:`
<table class="tbl">
<thead><tr>${cols.map(c => `<th>${esc(c.label)}</th>`).join(’’)}<th></th></tr></thead>
<tbody>
${list.map(e => `<tr> ${cols.map(c =>`<td>${c.val(e)}</td>`).join('')} <td><button class="btn-ghost" data-rh-view="${e.id}">Voir →</button></td> </tr> `).join(’’)}
</tbody>
</table>
`} </div> </div> `;
}
...
const empId = state.user.empId;
const body = `<div class="form-grid"> <div class="field full"><label class="field-label">Type d'absence</label> <select class="input" id="erType"> ${Object.entries(LEAVE_TYPES).map(([k,v]) =>`<option value="${k}">${esc(v.label)}</option>`).join('')} </select> </div> <div class="field"><label class="field-label">Du</label><input class="input" id="erStart" type="date" value="${dateISO(new Date())}"></div> <div class="field"><label class="field-label">Au</label><input class="input" id="erEnd" type="date" value="${dateISO(new Date())}"></div> <div class="field full"><label class="field-label">Motif (optionnel)</label><textarea class="input" id="erMotif" rows="3" placeholder="Pourquoi cette absence..."></textarea></div> <div class="field full text-mute" style="font-size:11.5px;">Ta demande sera envoyée à la direction. Tu seras notifié dès qu'elle sera traitée.</div> </div> `;
...
const body = `<div class="form-grid"> <div class="field"><label class="field-label">Entrée réelle</label><input class="input mono" id="sigIn" type="time"></div> <div class="field"><label class="field-label">Sortie réelle</label><input class="input mono" id="sigOut" type="time"></div> <div class="field full"><label class="field-label">Motif</label> <textarea class="input" id="sigReason" rows="3" placeholder="ex: pause étendue, oubli pointage..."></textarea> </div> </div> <div class="text-mute" style="font-size:12px;margin-top:10px;">Ta demande sera transmise à l'admin pour validation.</div>`;
const footer = `<div class="spacer"></div> <button class="btn-sec" data-close>Annuler</button> <button class="btn-pri" id="sigSend" style="width:auto;padding:10px 18px;">Envoyer</button>`;
...
if (!shifts.length) {
return `<div class="day-card off ${isToday?'today':''} ${fer?'ferie':''}"> <div><div class="day-num">${d.getDate()}</div><div class="day-name">${DAYS_SHORT[i]}</div></div> <div class="day-info"><div class="day-shift">Repos</div>${ferieTag}</div> </div>`;
}
return shifts.map(s => {
if (s.leaveType) {
const lt = LEAVE_TYPES[s.leaveType];
return `<div class="day-card ${isToday?'today':''} ${fer?'ferie':''}" style="border-left:3px solid currentColor;"> <div><div class="day-num">${d.getDate()}</div><div class="day-name">${DAYS_SHORT[i]}</div></div> <div class="day-info"> <div class="day-shift" style="font-family:var(--f-body);">${esc(lt?.label || 'Absence')}</div> <div class="day-meta">Non travaillé</div> ${ferieTag} </div> </div>`;
}
return ` <div class="day-card ${isToday?'today':''} ${fer?'ferie':''}"> <div><div class="day-num">${d.getDate()}</div><div class="day-name">${DAYS_SHORT[i]}</div></div> <div class="day-info"> <div class="day-shift">${s.start} → ${s.end}</div> <div class="day-meta">${esc(s.label || (s.type==='midi'?'Service midi':s.type==='soir'?'Service soir':'Journée'))} · ${shiftHours(s).toFixed(1)} h${s.pauseDuration?` · pause ${s.pauseDuration}mn`:''}</div> ${ferieTag} </div> </div>`;
}).join(’’);
}).join(’’);
...
`; } function profileRow(label, value) { return `<div class="row" style="padding:14px 16px;border-bottom:1px solid var(--c-line-2);justify-content:space-between;">
